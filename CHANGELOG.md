...
### 資料更新
- **W19 書單**（8 本）手動跑爬蟲並上線：5/7 覺醒中年、5/8 系統思考/自驱力、5/9 控制、5/10 養雞時代、5/11 燃燒殆盡、5/12 殺人者/告訴我你吃什麼

### 手動修正項目
- 《覺醒中年》GR count 163194 → 3194（用戶確認）
- 《你，燃燒殆盡了嗎？》日期 5/10 → 5/11
- 《控制》GR count 153,497,080 → 重抓後 3,497,092（符合 Gone Girl 實際規模）
- 《告訴我你吃什麼》Amazon 錯誤書（Ethel Scott）→ 清除

### Bug 修正：scraper/scrape.py
- **Goodreads 評論數解析**：舊 regex `([\d,]+)\s*ratings` 會抓頁面上第一個符合的大數字（如作者總評論數），改為優先比對 `avg rating — N ratings` 固定格式，備用才走 CSS selector，根除誤抓問題

### W19 最終 GR 資料（重抓後）
| 書名 | GR score | GR count |
|---|---|---|
| 覺醒中年 | 4.16 | 3,194 |
| 系統思考 | 3.8 | 137 |
| 控制 | 4.15 | 3,497,092 |
